    ...
    return (
        <>
                      ...
                      <div className="product-body product-action-inner">
                        <div className="order-summary">
                          <div className="d-flex justify-content-between">
                            <strong>نام محصول:</strong> <Link to={`/product/${item.product}`}><h5>{item.product_name}</h5></Link>
                          </div>
                          <div className="d-flex justify-content-between">
                            <strong>قیمت:</strong> <span>{item.product_price}</span>
                          </div>
                          <div className="d-flex justify-content-between">
                            <strong>فروشنده:</strong> <span>{item.product_seller}</span>
                          </div>
                          <div className="d-flex justify-content-between">
                            <strong>تعداد:</strong> <span>{item.quantity}</span>
                          </div>
                        </div>
                        <div className="d-flex justify-content-between">
                        <strong>رنگ:</strong>
            ...
            <div className="order-summary">
              ...
              <div className="d-flex justify-content-between">
                <strong>تاریخ:</strong> <span>{convertToPersianDate(order.payment.created_at)}</span>
              </div>
              <div className="d-flex justify-content-between">
                <strong>وضعیت سفارش:</strong> <span>{getStatusInPersian(order.status)}</span>
              </div>
              <div className="d-flex justify-content-between">
                <strong>کد پیگیری:</strong> <span>{order.payment.transaction_ref_id}</span>
              </div>
              <div className="d-flex justify-content-between">
